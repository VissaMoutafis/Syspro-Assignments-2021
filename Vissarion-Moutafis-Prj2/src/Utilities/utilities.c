...
// Argument parsing/checking generic functions


int error_i = 0;
char *error_string[] = {"", "Wrong Number of Arguments", "False Argument",
...
// <values> is the static array that the corresponding argument values are returned (malloc'd)
// <allowed_args> is the array of allowed argument flags
bool parse_args(int argc, char *argv[], char *values[], char *allowed_args[], int num_allowed_args) {
    // check for proper number of arguments
    int num_args = 1 + num_allowed_args * 2;
    if (argc != num_args) {
        error_i = 1;
        return false;
    }
    ...
    for (int i = 1; i < argc; i += 2) {
        int index = -1;
        if (check_arg(argv[i], allowed_args, num_allowed_args, &index)) {
            // index is the index number of the proper argument listing
            values[index] = argv[i + 1];

            // make sure that the value is not another argument, this cannot be
            // allowed
...
void print_arg_error(char *usage) {
    fprintf(stderr, "Error in arguments (%s)\n%s\n", error_string[error_i], usage ? usage : "-No Usage Message-");
}