...
static void check_fds(void *monitor, struct pollfd fds[], int nfd, int *active, MessageHandler handler, void *return_args[]) {
...
int monitor_get_response(void *_monitor, MessageHandler handler, int process_id, int fd, void *return_args[]) {
    ...
    while (active) {
        ...
        if (ret > 0) {
            check_fds(monitor, fds, 1, &active, handler, return_args);
        }
        if (ret < 0) {
            perror("poll");
            exit(1);
